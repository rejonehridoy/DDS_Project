...
BEGIN
	FOR R IN (select constant,coefficient from DatasetInfo where name = datasetName union select constant,coefficient from DatasetInfo@site1 where name = datasetName) LOOP
		constant := R.constant;
