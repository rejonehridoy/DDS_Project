CREATE OR REPLACE PACKAGE BODY dataset_display AS
	...
	BEGIN
		...
		FOR R IN (select dv.vid,dv.X,dv.Y from (select * from DatasetValues union select * from DatasetValues@site1) dv 
			inner join (select * from DatasetInfo union select * from DatasetInfo@site1) di on dv.did = di.did where 
