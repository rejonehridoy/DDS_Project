...
BEGIN
	...
	FOR R IN (select * from datasetinfo where name = dataset_name union select * from datasetinfo@site1 where name = dataset_name) LOOP
		isExist := 1;
	END LOOP;
	...
	IF isExist = 0 THEN
		...
		ELSE
			insert into DatasetInfo@site1 values(did,dataset_name,0,0);
