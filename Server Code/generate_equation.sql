...
BEGIN
	select count(vid) into n from (select vid from datasetvalues where did = id union select vid from datasetvalues@site1 where did = id);
	select sum(x),sum(y) into sum_x,sum_y from all_dataset_values where did = id;
	select avg(x),avg(y) into avg_x,avg_y from all_dataset_values where did = id;
	
	FOR R in (select X,Y from datasetvalues where did = id union select X,Y from datasetvalues@site1 where did = id) LOOP
		xx := xx + R.X * R.X;
		xy := xy + R.X * R.Y;
	...
	ELSE
		...
		update DatasetInfo@site1 set constant = a, coefficient = b where did = id;
	END IF;
