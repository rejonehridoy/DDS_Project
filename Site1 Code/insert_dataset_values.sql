CREATE OR REPLACE PACKAGE BODY dataset_insertion AS
	...
	BEGIN
		select count(vid) into temp from DatasetValues@server1;
		select count(vid) into vid from DatasetValues;
		IF temp > vid THEN
			vid := temp;
		END IF;
		vid := vid + 1;
	...
	BEGIN
		...
		select count(did) into isFound from DatasetInfo@server1 where name = datasetName;
		
		IF isFound = 1 THEN
			select did into did from DatasetInfo@server1 where name = datasetName;
			vid := generate_vid;
			dataset_storage := 0;	-- 0 for server
			insert into DatasetValues@server1 values(vid,did,x_value,y_value);
